# ===== 셀 3: FAISS DB 실제 문서 기반 자연스러운 분포 테스트 =====
import json
import os
import sys
import time
...
print("\n🧪 FAISS DB 실제 문서 기반 자연스러운 분포 테스트")
print("-" * 70)
...
try:
    ...
    for i, doc in enumerate(collected_docs[:10]):  # 상위 10개 문서만 사용
        ...
        test_cases.append({
            ...
            "expected_pattern": "FAISS DB 기반 카테고리별 자연스러운 분포",
    ...
    test_cases = [
        {
            ...
            "expected_pattern": "부정 질문 → 주로 Yes 답변 (8:2 또는 9:1)",
        ...
        },
        {
            ...
            "expected_pattern": "긍정 질문 → 주로 Yes 답변 (8:2 또는 9:1)",
...
# FAISS DB 기반 자연스러운 분포 테스트 결과 저장
faiss_based_results = []
...
try:
        ...
        try:
            ...
            if result and result.questions:
                ...
                if yes_count == 10 or no_count == 10:
                    improvement_status = "❌ 완전 편향 (10:0) - 개선 실패"
                    improvement_score = 0
                elif yes_count >= 9 or no_count >= 9:
                    improvement_status = "🌟 우수한 자연 분포 (9:1)"
                    improvement_score = 5
                elif yes_count >= 8 or no_count >= 8:
                    improvement_status = "✨ 좋은 자연 분포 (8:2)"
                    improvement_score = 4
                elif yes_count >= 7 or no_count >= 7:
                    improvement_status = "📊 적당한 분포 (7:3)"
                    improvement_score = 3
                elif yes_count >= 6 or no_count >= 6:
                    improvement_status = "⚖️ 균형적 분포 (6:4)"
                    improvement_score = 2
                else:
                    improvement_status = "🎯 완전 균형 분포 (5:5)"
                    improvement_score = 1
    ...
    # 전체 결과 요약
    print("\n" + "=" * 70)
    print("🏁 FAISS DB 기반 자연스러운 분포 테스트 완료!")
    print("=" * 70)
    ...
    final_save_data = {
        ...
        "test_type": "FAISS DB 기반 자연스러운 분포 테스트",
    ...
    if successful_tests:
        ...
        print(f"✅ FAISS DB 기반 테스트 완료. 다음 셀에서 {len(generated_questions)}개 질문으로 RAG 비교 테스트를 진행할 수 있습니다.")
