...
class RAGEvaluator:
    ...
    def evaluate_topk_performance(self, test_cases: List[Dict], topk_values: List[int], alpha_values: List[float]):
        results = []
        
        for top_k in tqdm(topk_values, desc="Top-k 실험 중"):
            for alpha in alpha_values:
                logger.info(f"Top-k={top_k}, Alpha={alpha} 실험 시작")
                ...
                if case_scores:
                    avg_scores = {
                        metric: np.mean([score[metric] for score in case_scores])
                        for metric in ['context_recall', 'context_precision', 'context_entity_recall']
                    }
                    ...
                    logger.info(f"Top-k={top_k}, Alpha={alpha} 결과:")
                    logger.info(f"Context Recall: {avg_scores['context_recall']:.3f}")
                    logger.info(f"Context Precision: {avg_scores['context_precision']:.3f}")
    ...
    def visualize_topk_results(self, results_df: pd.DataFrame):
        ...
        for idx, metric in enumerate(metrics, 1):
            ...
            plt.title(f'{metric} by Top-k and Alpha')
            plt.xlabel('Alpha')
            plt.ylabel('Top-k')
        
        # 2. Top-k별 평균 성능
        plt.subplot(2, 2, 4)
        avg_by_topk = results_df.groupby('top_k')[metrics].mean()
        avg_by_topk.plot(kind='bar')
        plt.title('Average Performance by Top-k')
        plt.xlabel('Top-k')
        plt.ylabel('Score')
