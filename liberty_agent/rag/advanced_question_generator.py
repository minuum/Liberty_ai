...
class AdvancedQuestionGenerator:
    def __init__(self, model_name: str = "gpt-4o-2024-08-06", temperature: float = 0.1):
        self.llm = ChatOpenAI(model_name=model_name, temperature=temperature)
        self.results_dir = Path("question_generation_results")
        self.results_dir.mkdir(exist_ok=True)
        # KeyBERT 모델은 초기화 시 로드 (메모리 효율성)
        try:
    ...
    def analyze_document(self, document: str) -> DocumentAnalysis:
        ...
        analysis_prompt = ChatPromptTemplate.from_messages([
            ("system", """법률 전문가로서 주어진 문서를 분석하여 다음 요소들을 추출하세요.
    ...
    def generate_standard_questions_with_policy(
        ...
        if hybrid_policy_level == 1:
            policy_instruction = f"다음 핵심 법률용어들을 반드시 질문에 포함하고, 전문 용어를 그대로 사용하여 주십시오: {', '.join(keywords_to_consider)}. (전문가 및 전공자 대상)"
        elif hybrid_policy_level == 2:
            policy_instruction = f"다음 주요 키워드({', '.join(keywords_to_consider)})를 유지하되, 일부 보조적이거나 어려운 키워드는 쉬운 유의어 또는 생활어로 대체하여 질문을 생성해 주십시오. (대학생 및 직장인 중급자 대상)"
        elif hybrid_policy_level == 3:
            policy_instruction = f"다음 키워드 중 절반 이하만 사용하거나 쉬운 말로 바꾸고, 문장 전체를 단순화하여 질문을 생성해 주십시오: {', '.join(keywords_to_consider)}. (일반 성인 및 초심자 대상)"
        elif hybrid_policy_level == 4:
            policy_instruction = f"법률 용어는 가급적 모두 제거하고, 문서의 핵심 맥락만 유지하여 매우 쉬운 생활 문장으로 질문을 생성해 주십시오. (학생 및 비전문가 대상, 참고 키워드: {', '.join(keywords_to_consider)})"
        else:
            policy_instruction = f"다음 주요 키워드({', '.join(keywords_to_consider)})를 참고하여 다양한 난이도의 질문을 생성해주세요."

        question_prompt_template = ChatPromptTemplate.from_messages([
            ("system", f"""법률 전문가로서 주어진 문서에 대한 질문을 생성하세요.
    ...
    def generate_boosted_questions(
        ...
        system_prompt_content = "당신은 고도로 숙련된 법률 교육 콘텐츠 개발자입니다. 주어진 법률 문서와 분석 내용을 바탕으로, 비판적 사고와 깊이 있는 이해를 요구하는 정교한 법률 질문을 생성해야 합니다."
        ...
        if boost_strategy == "analytical_depth":
            ...
            """
            human_prompt_content = f"""**제공된 법률 문서:**
            {document_text[:3000]}... (문서가 매우 길 경우 일부만 표시)

            **문서 분석 요약:**
            - 핵심 내용: {analysis_result.key_points}
            - 주요 법적 쟁점: {analysis_result.legal_issues}
            - 문서 난이도: {analysis_result.complexity_level}
            ...
            **생성할 질문 형식 (QuestionSet 스키마 준수):**
            {{
                "questions": [  # <--- 여기에 {{ "questions" }} 가 있습니다.
                    {{
                        "question": "...",
                        "reasoning": "...",
        ...
        try:
            ...
            response = chain.invoke({"document_text": document_text, "primary_theme": primary_theme, "num_questions": num_questions}) # num_questions 추가
